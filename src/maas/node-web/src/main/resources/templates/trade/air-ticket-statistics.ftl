...
<div class="layui-tab">
    ...
    <p style="color: #26bec5;">报表统计 > 机票统计</p>
    ...
    <form class="layui-form" action="">
        <div class="layui-form-item"  >
            ...
            <div class="layui-inline">
                ...
                <div class="layui-input-block select">
                    <select name="countWay" lay-filter="countWay">
                        <option value="订单类别" selected="">订单类别</option>
                        <option value="出票员" >出票员</option>
                        <option value="订单类型">订单类型</option>
                        <option value="订单来源">订单来源</option>
                        <option value="来源方式">来源方式</option>
                    </select>
                </div>
            </div>
        ...
        <div class="layui-form-item" id="secondRow">
            ...
            <div class="layui-inline" style="display: none">
                ...
                <div class="layui-input-block select">
                    <select name="出票" lay-filter="aihao" id="orderType">
    ...
    <#--</div>-->
        <div class="layui-form-item">
            ...
            <div class="layui-inline">
                <button class="layui-btn layui-btn-radius" lay-submit style="height: 27px;line-height: 27px;" id="exportBtn" lay-filter="exportBtn">导出</button>
            </div>
        </div>
    ...
    layui.use(['form','element','laydate',/*'vip_table',*/'table' ,'laytpl'], function () {
        ...
        var options = {
            ...
            , cols: [[                  //标题栏
                ...
                , {field: 'countWay', title: '订单类别', width: 120}
                , {field: 'total', title: '总订单量', width: 100 ,templet:'#totalTpl'}
                , {field: 'accomplish', title: '已完成量', width: 100,templet:'#accomplishTpl'}
                , {field: 'notAccomplish', title: '未完成量', width: 100,templet:'#notAccomplishTpl'}
                , {field: 'rob', title: '抢单量 ', width: 100,templet:'#robTpl'}
                , {field: 'designate', title: '派单量', width: 100,templet:'#designateTpl'}
                , {field: 'tempDeposit', title: '暂存量', width: 100,templet:'#tempDepositTpl'}
                , {field: 'overTime', title: '超时量', width: 100,templet:'#overTimeTpl' }
                , {field: 'sendBack', title: '退回量', width: 100,templet:'#sendBackTpl'}
                , {field: 'withdraw', title: '抽回量', width: 100,templet:'#withdrawTpl'}

            ]]
            , id: 'dataCheck'
            // , url:LAYUI_FILE_PATH+'/assets/json/order_table.json'// '/api/order/list'
            ,url:_PATH+"/api/orderRecord/excelExportCountWayByCondition"
            , method: 'post'
            // , method: 'get'//'post'
        ...
        form.on('submit(formDemo)', function(data){
            ...
            options.cols[0][1].title = data.field.countWay;
        ...
        form.on("select(countWay)", function (data) {
        ...
        form.on('submit(exportBtn)', function(data){
            ...
            var formdtat = new FormData();
            formdtat.append("countWay", data.field.countWay);
            formdtat.append("orderSort", data.field.orderSort);
            $.ajax({
                url:_PATH+"/api/orderRecord/countExport",
                data:formdtat,
                // contentType: false,//form表单提交这里
                // processData: false,//form表单提交这两个一定设置为false
                contentType: 'application/json',
        ...
        $("body").on("click", ".totalBtn",function(){
            ...
            window.parent.addTab_parent(_PATH+"/trade/air-ticket-total.html","总的订单量",layer);

        });
        //总的订单量
        $("body").on("click", ".accomplishBtn",function(){


            window.parent.addTab_parent(_PATH+"/trade/air-ticket-accomplish.html","已完成订单量",layer);

        });
        //总的订单量
        $("body").on("click", ".notAccomplishBtn",function(){


            window.parent.addTab_parent(_PATH+"/trade/air-ticket-notaccomplish.html","未完成订单量",layer);

        });

        //总的订单量
        $("body").on("click", ".robBtn",function(){
            window.parent.addTab_parent(_PATH+"/trade/air-ticket-ge.html?type=抢单量","抢单量",layer);
        });
        $("body").on("click", ".designateBtn",function(){
            window.parent.addTab_parent(_PATH+"/trade/air-ticket-ge.html?type=派单量","派单量",layer);
        });
        $("body").on("click", ".tempDepositBtn",function(){
            window.parent.addTab_parent(_PATH+"/trade/air-ticket-ge.html?type=暂存量","暂存量",layer);
        });
        $("body").on("click", ".overTimeBtn",function(){
            window.parent.addTab_parent(_PATH+"/trade/air-ticket-ge.html?type=超时量","超时量",layer);
        });
        $("body").on("click", ".sendBackBtn",function(){
            window.parent.addTab_parent(_PATH+"/trade/air-ticket-ge.html?type=退回量","退回量",layer);
        });
        $("body").on("click", ".withdrawBtn",function(){
            window.parent.addTab_parent(_PATH+"/trade/air-ticket-ge.html?type=抽回量","抽回量",layer);
        });
    });
...
<script  type="text/html" id="totalTpl">
    ...
    <span style="color: red;" class="totalBtn">{{d.total}}</span>
</script>
<script  type="text/html" id="accomplishTpl">

    <span style="color: red;" class="accomplishBtn">{{d.accomplish}}</span>
</script>
<script  type="text/html" id="notAccomplishTpl">

    <span style="color: red;" class="notAccomplishBtn">{{d.notAccomplish}}</span>
</script>


<script  type="text/html" id="robTpl">
    <span style="color: red;" class="robBtn">{{d.rob}}</span>
</script>
<script  type="text/html" id="designateTpl">
    <span style="color: red;" class="designateBtn">{{d.designate}}</span>
</script>
<script  type="text/html" id="tempDepositTpl">
    <span style="color: red;" class="tempDepositBtn">{{d.tempDeposit}}</span>
</script>
<script  type="text/html" id="overTimeTpl">
    <span style="color: red;" class="overTimeBtn">{{d.overTime}}</span>
</script>
<script  type="text/html" id="sendBackTpl">
    <span style="color: red;" class="sendBackBtn">{{d.sendBack}}</span>
</script>
<script  type="text/html" id="withdrawTpl">
    <span style="color: red;" class="withdrawBtn">{{d.withdraw}}</span>
</script>
<!-- 表格操作按钮集 -->
