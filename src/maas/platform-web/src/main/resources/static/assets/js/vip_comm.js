...
var parentElement,$$;
// 配置
console.log("====================",LAYUI_FILE_PATH);
layui.config({
    base: LAYUI_FILE_PATH+'/assets/js/'  // 模块目录
}).extend({                     // 模块别名
    vip_nav: 'vip_nav'
    , vip_tab: 'vip_tab'
    , vip_table: 'vip_table'
...
layui.use(['layer', 'element', 'util'], function () {
    ...
    //获取当前tab
    window.getThisNavTab = function (url){
        console.log("===",url);
        console.log($(document).find('body .my-side ul').find('li'));
        var temp;
        $(document).find('body .my-side ul').find('li').each(function(index,a){
            console.log("=====",a);

            $(a).find("dl").find("dd").each(function(k,j){
                $(j).each(function(m,n){
                    $(n).siblings().removeClass("layui-this");
                    if($(n).find('a').attr("href-url") == url){
        ...
        return  $(document).find('body .my-side ul').find('li')
    }
    ...
    // 皮肤
    function skin() {
        var skin = localStorage.skin ? localStorage.skin : 1;
        var body = $('body');
...
//添加TAB选项卡
function addTab_parent(url,title,layer){
    var card    = 'card';                                   // 选项卡对象
...
function getTitleId_children(card,title){
    var id = -1;
    $$(document).find(".layui-tab[lay-filter=" + card + "] ul li").each(function(){
        console.log("lay-id",$$(this).attr('lay-id'));
        if(title === $$(this).find('span').html()){
            id = $$(this).attr('lay-id');
        }
    });
